...
## nginx pod 배포 
kubectl apply -f lec2/deploy.yaml
...
```
```sh
k apply -f lec2/clusterip-svc.yaml
...
curl http://{server-ip}:30001  ## master-1
...
```
```sh
k apply -f lec2/loabalancer-svc.yaml
## pending 확인
```
...
```
```sh
k apply -f lec2/external-svc.yaml
...
# clear
```sh
k delete -f lec2/loabalancer-svc.yaml
k delete -f lec2/deploy.yaml
k delete -f lec2/external-svc.yaml
